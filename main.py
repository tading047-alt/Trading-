...
# -*- coding: utf-8 -*-
"""
🚂 نظام اكتشاف الانفجارات - الإصدار المتكامل مع Backtesting
First Station Explosion Detector - Ultimate Edition with Backtesting
...
# =========================================================
# 🆕 وضع التشغيل (live أو backtest)
# =========================================================
TRADING_MODE = os.environ.get("TRADING_MODE", "live")  # live أو backtest
BACKTEST_DAYS = int(os.environ.get("BACKTEST_DAYS", 7))  # عدد أيام الاختبار العكسي
...
# =========================================================
# كاشف الانفجارات (مع جميع الفلاتر) - يُستخدم في Live و Backtest
# =========================================================
class ExplosionDetector:
    ...
    # --- دوال RSI, MACD, الأنماط, الأولوية (مطابقة للإصدار السابق) ---
...
╚══════════════════════════════════════════════════════════╝
        ...
        for symbol in symbols[:50]:  # نختبر أول 50 عملة للسرعة
            try:
                ...
                for i in range(100, len(ohlcv)):
                    # محاكاة تحليل الإشارة عند كل شمعة
                    current_slice = ohlcv[:i+1]
                    current_price = current_slice[-1][4]
                    ...
                    # تحليل إشارة جديدة
                    data = np.array(current_slice[-60:])
                    if len(data) < 30: continue
                    
                    closes = data[:,4]; volumes = data[:,5]
                    avg_vol = np.mean(volumes[-20:]) if len(volumes) >= 20 else volumes[-1]
                    vol_ratio = volumes[-1] / avg_vol if avg_vol > 0 else 0
                    if vol_ratio < VOLUME_RATIO_THRESHOLD: continue
                    
                    # فحص سريع للأنماط
                    detected = []
                    calm = self.detector._check_calm_before_storm(volumes, closes)
                    whale = self.detector._check_whale_accumulation(volumes, closes)
                    boll = self.detector._check_bollinger_squeeze(closes)
                    for check in [calm, whale, boll]:
                        if check['detected']:
                    ...
                    if len(detected) >= MIN_PATTERNS_REQUIRED:
                        ...
                        signal = ExplosionSignal(
                            ...
                            patterns=detected, volume_24h=500000,
                            current_change=0, priority=3
                        )
...
╚══════════════════════════════════════════════════════════╝
        ...
        return {'total_trades': total_trades, 'winning_trades': winning_trades, 
...
class ExplosionScannerEngine:
    ...
    async def run(self):
        ...
        if TRADING_MODE == "backtest":
            print("║     📊 نظام الانفجارات – وضع Backtesting 💥      ║")
        else:
            print("║     💥 نظام الانفجارات المتكامل – Binance 💥      ║")
        print("╚══════════════════════════════════════════════════════════╝")
        ...
        # 🆕 تشغيل Backtesting إذا كان الوضع = backtest
        if TRADING_MODE == "backtest":
            backtest_engine = BacktestEngine(self.detector, self.trade_manager)
            await backtest_engine.run(exchange)
            await exchange.close()
            return  # انتهاء البوت بعد الـ Backtest

        # 👇 Live Trading
        await self.notifier.send_startup_message(mode="live")
        while True:
            try:
